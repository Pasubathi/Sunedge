...
import WooComFetch, { getBaseUrl, getUrl, postHttp, postFormHttp, getHttp } from '../common/WooComFetch'
...
class Login extends PureComponent {
  ...
  login = async t => {
    ...
    let loginData = {
              "distributor_id": this.state.userName,
              "password": this.state.password,
              "loginuser":"adminapsun",
              "loginpass":"56tyghefgr(jgdnklo0$"
            }
    const data = await postHttp('https://sunedgemkt.in/appmod/user_login.php', loginData)
    console.log('login data',data);
    if (data.status == 1) {
      t.getUserData(data, 'simple', t)
    }
    if (data.status == 0) {
      t.setState({ errorMessage: 'Wrong app username or password', SpinnerTemp: false, colorGren: false })
    }
    if (data.status == 2) {
      t.setState({ errorMessage: 'Wrong distributor id or password', SpinnerTemp: false, colorGren: false })
    }
  }
...
signIn = async () => {
  ...
  EmailNumberCheck () {
    ...
    return (
      (userName.length == 0) || reg.test(userName) === false
    )
  }
    ...
    dispatch(async dispatch => {
      ...
      if (SyncStorage.get('cartScreen') === 1) {
        ...
        if (SyncStorage.get('drawerLogin')) {
          ...
          t.props.navigation.navigate('Home3Screen')
        }
      }
